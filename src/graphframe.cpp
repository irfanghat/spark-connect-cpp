...
const DataFrame& GraphFrame::vertices() const
{
    return vertices_;
}
const DataFrame& GraphFrame::edges() const
{
    return edges_;
}
 ...
 *        Relation.extension` field (`tag 998`).  Returns the resulting
 * DataFrame.
 *
...
void GraphFrame::setExprCol(ColumnOrExpression* coe, const spark::sql::functions::Column& col)
{
    // --------------------------------------------------------------------------
...
DataFrame GraphFrame::pageRank(double reset_probability, std::optional<int> max_iter,
                               std::optional<double> tol, std::optional<std::string> source_id_str,
...
DataFrame GraphFrame::parallelPersonalizedPageRank(const std::vector<std::string>& source_ids_str,
                                                   double reset_probability, int max_iter) const
{
    GraphFramesAPI api;
...
DataFrame GraphFrame::parallelPersonalizedPageRank(const std::vector<int64_t>& source_ids_long,
                                                   double reset_probability, int max_iter) const
{
    GraphFramesAPI api;
    auto* pppr = api.mutable_parallel_personalized_page_rank();
...
DataFrame GraphFrame::connectedComponents(const std::string& algorithm, int checkpoint_interval,
...
DataFrame GraphFrame::bfs(const std::string& from_expr, const std::string& to_expr,
                          const std::string& edge_filter, int max_path_length) const
{
    GraphFramesAPI api;
...
DataFrame GraphFrame::powerIterationClustering(int k, int max_iter,
...
DataFrame GraphFrame::svdPlusPlus(int rank, int max_iter, double min_value, double max_value,
                                  double gamma1, double gamma2, double gamma6, double gamma7) const
{
    GraphFramesAPI api;
