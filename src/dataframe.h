...
/**
 * @class DataFrame
 * @brief Represents a distributed collection of data organized into named
 * columns.
 *
 * This class is the primary entry point for working with data in a Spark
 * Connect application. It provides methods to inspect the data and trigger
 * execution of the underlying logical plan. It is immutable and transformations
 * on a DataFrame return a new DataFrame with an updated plan.
 */
class DataFrame
{
  public:
     ...
     * @param stub A shared pointer to the gRPC stub for communicating with
     * the Spark server.
     * @param plan The logical plan representing the DataFrame's
     * transformations.
    ...
    DataFrame(std::shared_ptr<spark::connect::SparkConnectService::Stub> stub,
              spark::connect::Plan plan, std::string session_id, std::string user_id);
    ...
    /**
     * @brief Gets the current plan. Returns by const reference to avoid
     * copies.
     */
     ...
     *
     * This method prints the data returned from a Spark SQL query or
     * transformation. Internally, it deserializes the Arrow RecordBatch
     * received from the Spark server and formats it for terminal output.
     *
     * @param limit The maximum number of rows to display. If not provided,
     * all rows are shown. Defaults to 10 if available.
     *
     ...
     * server.
     *
     * @note This operation requires a round-trip to the Spark server via
     * AnalyzePlan RPC.
    ...
    /**
     * @brief Projects a set of `Column` expressions and returns a new
     * DataFrame.
     * @param cols A vector of `Column` objects (supporting aliases, math,
     * etc.).
     * @example auto filtered_df = df.filter(col("name") == lit("Alice"))
     *                     .select({col("name"),
     * _col.alias("age_plus_one")});
     */
     ...
     * make no guarantee about the backward compatibility of the schema of
     * the resulting `DataFrame`. Use `summary` for expanded statistics and
     * control over which statistics to compute.
     * @param cols Column name or list of column names to describe by
     * (default All columns).
     * @example
     ...
     *  |    max|  13|
     *  +-------+----+
     * @returns A new DataFrame that describes (provides statistics) given
     * DataFrame.
     */
     ...
     *
     * @param statistics Column names to calculate statistics by (default
     * All columns).
     * @returns A new `DataFrame` that provides statistics for the given
     * `DataFrame`.
     */
    ...
    /**
     * @brief Performs an inner join with another DataFrame on all columns
     * with matching names.
     *
     * This function automatically finds all columns that exist in both
     * DataFrames and performs an inner join on them. If there are no common
    ...
    /**
     * @brief Joins this DataFrame with another DataFrame using a join
     * column name.
     *
     * Performs an equi-join using the specified column. The column must
    ...
    /**
     * @brief Joins this DataFrame with another DataFrame using multiple
     * column names.
     *
    ...
    /**
     * @brief Joins this DataFrame with another DataFrame using a SQL
     * expression.
     *
     * @param other The right side of the join.
    ...
    /**
     * @brief Returns a new `DataFrame`. Concise syntax for chaining custom
     * transformations.
     * @tparam F A callable type that takes a DataFrame as the first
     * argument.
    ...
    {
        // --------------------------------------------------------------------
        // Invoke the function with this DataFrame and any additional
        // arguments
        // --------------------------------------------------------------------
    ...
    /**
     * @brief Returns a new `DataFrame` by adding a column or replacing the
