 ...
 * destination vertex IDs of edges, respectively.
 *
 */
class GraphFrame
{
  public:
     ...
     * @return A new DataFrame with one column per named vertex/edge in the
     * pattern.
     */
    DataFrame find(const std::string& pattern) const;

    /**
     * @brief The triplets (source vertex)-[edge]->(destination vertex) for
     * all edges in the graph. Returns a DataFrame of all triplets (src
     * vertex, edge, dst vertex).
     * @return DataFrame with columns: `src`, `edge`, `dst`.
     */
    DataFrame triplets() const;

    /**
     * @brief Returns a sub-graph containing only edges satisfying
     * `condition`.
    ...
    /**
     * @brief Filters the vertices based on expression, remove edges
     * containing any dropped vertices. Returns a sub-graph containing only
     * vertices satisfying `condition`.
     * @param condition  String or Column describing the condition
    ...
    // Reference:
    // https://graphframes.io/api/python/graphframes.html#graphframes.GraphFrame.k_core
    // =======================================================================

    /**
     * @brief Runs PageRank until convergence or for a fixed number of
     * iterations on the graph.
     *
     * Exactly one of `max_iter` or `tol` must be specified:
     *  - `max_iter` runs a fixed number of iterations.
     *  - `tol`      runs until the L1 norm of the rank change falls below
     * the tolerance.
     *
     * @param reset_probability  Probability of a `teleport` (1 - damping
     * factor). Default 0.15.
     * @param max_iter           Fixed iteration count (mutually exclusive
     * with tol).
     * @param tol                Convergence tolerance (mutually exclusive
     * with max_iter).
     * @param source_id_str      Optional source vertex id (string).
     * Personalises PageRank.
     * @param source_id_long     Optional source vertex id (integer).
     * Personalises PageRank.
     * @return DataFrame of vertices with "pagerank" column and edges with
     * `weight`.
     */
    DataFrame pageRank(double reset_probability = 0.15, std::optional<int> max_iter = std::nullopt,
    ...
    /**
     * @brief Run the personalized PageRank algorithm on the graph, from the
     * provided list of sources in parallel for a fixed number of
     * iterations.
     *
     * @param source_ids_str   String vertex IDs used as personalisation
     * sources.
     * @param reset_probability  Teleport probability.
     * @param max_iter         Number of iterations.
     * @return Vertex DataFrame with one pagerank column per source.
     */
    DataFrame parallelPersonalizedPageRank(const std::vector<std::string>& source_ids_str,
                                           double reset_probability = 0.15,
                                           int max_iter = 10) const;

    /** @overload Integer source IDs. */
    DataFrame parallelPersonalizedPageRank(const std::vector<int64_t>& source_ids_long,
                                           double reset_probability = 0.15,
                                           int max_iter = 10) const;
     ...
     * @param broadcast_threshold  Threshold for broadcasting small graphs
     * (default 1 000 000).
     * @return Vertex DataFrame with a "component" column (long).
     */
    DataFrame connectedComponents(const std::string& algorithm = "graphframes",
    ...
    /**
     * @brief Runs the Label Propagation Algorithm (LPA) for community
     * detection. Runs static label propagation for detecting communities in
     * networks.
     ...
     * vertices neighborhoods. It requires to collect the whole neighborhood
     * of each vertex.
     ...
     *
     * @param from_expr       SQL expression selecting source vertices (e.g.
     * `id = 1`).
     * @param to_expr         SQL expression selecting destination vertices.
     * @param edge_filter     Optional SQL expression to filter traversable
     * edges.
     * @param max_path_length Maximum number of edges in the path (default
     * 10).
     * @return DataFrame containing columns for each vertex and edge along
     * the shortest path.
     */
    DataFrame bfs(const std::string& from_expr, const std::string& to_expr,
                  const std::string& edge_filter = "", int max_path_length = 10) const;
     ...
     * Proceedings of Simpósio Brasileiro de Pesquisa Operacional (SBPO’15)
     * (2015): 1-11.
     * @return Vertex DataFrame annotated with cycle membership (A DataFrame
     * with unique cycles).
     */
     ...
     * The k-core is the maximal subgraph such that every vertex has at
     * least degree k. The k-core metric is a measure of the centrality of a
     * node in a network, based on its degree and the degrees of its
     * neighbors. Nodes with higher k-core values are considered to be more
     * central and influential within the network. This implementation is
     * based on the algorithm described in: Mandal, Aritra, and Mohammad Al
     * Hasan. `A distributed k-core decomposition algorithm on spark.` 2017
     ...
     */
    DataFrame powerIterationClustering(int k, int max_iter = 20,
     ...
     */
    DataFrame svdPlusPlus(int rank = 10, int max_iter = 2, double min_value = 0.0,
                          double max_value = 5.0, double gamma1 = 0.007, double gamma2 = 0.007,
                          double gamma6 = 0.005, double gamma7 = 0.015) const;
  ...
  private:
    ...
    /**
     * @brief Populates a ColumnOrExpression from a plain SQL string (`expr
     * variant`).
     */
     ...
     *        by serialising the Column's inner Expression proto to bytes
     * (`col variant`).
     */
