...
TEST_F(SparkIntegrationTest, ReadTextFileWithOptions)
{
    ...
    // -------------------------------------------------------------------------------------------
    // Reading text files is a straightforward way to ingest unstructured or
    // semi-structured data, transforming plain text into DataFrames with
    // the flexibility of Spark’s distributed engine.
    //
    // auto df = spark->read()
...
TEST_F(SparkIntegrationTest, ReadParquetWithOptions)
{
    ...
    //
    // "mergeSchema" - Merges schemas across all Parquet part-files which
    // results in expensive operations "datetimeRebaseMode" - Handles legacy
    // dates from older Spark versions
    // ----------------------------------------------------------------------------
...
TEST_F(SparkIntegrationTest, HandleMissingFile)
{
    ...
    // -----------------------------------------------------------------------------
    // Show triggers an ExecutePlan gRPC call which should fail in this
    // scenario
    // -----------------------------------------------------------------------------
 ...
 * * These options control how the underlying Spark engine parses source files
 * (CSV, JSON, etc.) into C++ Row objects. Use these keys within a
 * std::map<std::string, std::string> passed to the .options() method.
 *
 * Schema Discovery & Integrity
 * - @b "inferSchema": (Boolean) If "true", Spark performs a first pass over the
 * data to automatically determine column types (e.g., Integer, Double). If
 * "false" (default), all columns are treated as StringType.
 * - @b "header": (Boolean) If "true", the first line of the file is used for
 * column names. If "false", columns are named automatically (e.g., _c0, _c1).
 * - @b "enforceSchema": (Boolean) If "true", forces the data to match a
 * user-defined schema, returning null for non-compliant fields.
 *
 * CSV Parsing Configuration
 * - @b "delimiter": (String) The character separating fields. Default is ','.
 * - @b "quote": (String) The character used for escaping values containing the
 * delimiter. Default is '"'.
 * - @b "escape": (String) The character used for escaping quotes inside a
 * value. Default is '\'.
 * - @b "nullValue": (String) Defines which string represents a NULL in the
 * dataset.
 *
 * Temporal (Date/Time) Formats
 * - @b "dateFormat": (String) Pattern used to parse DateType columns (e.g.,
 * "yyyy-MM-dd").
 ...
 * * @note When using CSVs without a manual schema, @ref inferSchema must be
 * "true" for Row::get_long() or Row::get_double() to succeed, as it prevents
 * numeric data from being stored as std::string in the internal variant.
 */
